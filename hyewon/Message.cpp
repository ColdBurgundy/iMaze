...
void displayBMP() {
    ...
    glBegin(GL_QUADS);
    // 세로 크기를 살짝 늘림 (y축 끝 좌표를 50 픽셀 증가)
    glTexCoord2f(0.0f, 0.0f); glVertex2f(200, 50);   // 왼쪽 아래
    glTexCoord2f(1.0f, 0.0f); glVertex2f(800, 50);  // 오른쪽 아래
    glTexCoord2f(1.0f, 1.0f); glVertex2f(800, 400); // 오른쪽 위 
    glTexCoord2f(0.0f, 1.0f); glVertex2f(200, 400);  // 왼쪽 위 
    glEnd();
